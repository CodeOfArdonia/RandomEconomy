...
public class SystemStationCustomerScreenHandler extends ScreenHandler {
    ...
    public SystemStationCustomerScreenHandler(int syncId, PlayerInventory playerInventory, Inventory left, Inventory right, ScreenHandlerContext context) {
        super(NeeScreenHandlers.SYSTEM_STATION_CUSTOMER, syncId);
        checkSize(left, 12);
        checkSize(right, 12);
        this.left = left;
        this.right = right;
    ...
    static {
        ...
        ServerPlayNetworking.registerGlobalReceiver(Constants.SYSTEM_TRADE, (server, player, handler, buf, sender) -> {
            if (player.currentScreenHandler instanceof SystemStationCustomerScreenHandler h)
