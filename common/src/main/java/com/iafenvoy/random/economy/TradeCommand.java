        ...
        CommandRegistrationEvent.EVENT.register((dispatcher, access, env) -> dispatcher.register(CommandManager.literal("trade")
                ...
                .then(CommandManager.argument("target", EntityArgumentType.player())
                        .executes(ctx -> {
                            ...
                            self.sendMessage(Text.translatable("command.random_economy.trade_request_sent"));
                            target.sendMessage(Text.translatable("command.random_economy.trade_request", self.getDisplayName()).fillStyle(Style.EMPTY.withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/trade " + self.getGameProfile().getName()))));
                            return 1;
                        })
                )));
